...
- ✅ [理解 Transformer（第一部分）：为什么 RNN 几乎不可能训练](https://medium.com/data-science-collective/transformers-part-1-why-rnns-are-nearly-impossible-to-train-30d1986f5960)
- **📌 理解 Transformer（第二部分）：LSTM 如何（基本）修复循环神经网络（这篇博文！）**
...
1. **RNN 第一部分回顾**
2. **LSTM 来帮忙啦！** *我们试图解决的问题（暂且不用数学方法）*
3. **LSTM 单元的官方示意图（看起来有点吓人）** *我们介绍 1 个单元和 3 个门。*
4. **用实际数字进行讲解。** *你会发现数学其实并没有那么可怕。*
5. 遗忘门
6. 输入门
7. 输出门
8. **为什么 LSTM 比 RNN 更好？**
9. **问题一：** *训练 LSTM 仍然* ***非常慢。***
10. **问题二：** *LSTM 仍然* ***难以处理长序列。***
11. **接下来会发生什么：** Transformer 摒弃了循环往复的设定……而且成功了！

## RNN 第一部分回顾

如果你错过了[**本系列的第一部分**](https://medium.com/data-science-collective/transformers-part-1-why-rnns-are-nearly-impossible-to-train-30d1986f5960)，我们深入探讨了循环神经网络（RNN）的世界。RNN 非常巧妙，它们将记忆的概念引入了神经网络。但是，它们在大规模处理海量数据时表现不佳。
