...
- **📌 理解 Transformer（第三部分）：位置编码和词嵌入（*这篇博文！*）**
- 理解 Transformer（第四部分）：注意力机制其实就是几个矩阵（即将推出！）
- 理解 Transformer（第五部分）：最后几个安静的层正在默默地承担着繁重的工作（敬请期待！）

## 本博客将涵盖哪些内容？

1. **Transformer 热潮。** *你最近在网上接触到的一切背后的那个模型*
2. **第一部分和第二部分中 RNN 和 LSTM 的回顾。**
3. **为什么我们今天只讲嵌入和位置。** *（而不是一头扎进混乱里）*
4. **在确定位置之前，我们需要嵌入。**
5. **没有位置信息的问题。**
6. **一个糟糕的位置编码示例。**
7. **正弦和余弦解法。**
...
📢 这篇文章里，我们聊了 Transformer 其实并不能“理解”词序——除非我们明确地教它。我们走了一遍词嵌入的局限、保住顺序的重要性，以及为什么正弦波和余弦波最终成了完美的解法。一路都配着可视化、向量，还有刚好够用、不至于无聊的三角函数。

但现在，我想把话筒递给你。

- 你有没有把词嵌入可视化过，或者动手玩过自定义的位置编码？
- 你觉得正弦编码直观吗？还是说这个想法仍然有点抽象？
- 又或者，这是你第一次真正深入注意力机制启动*之前*发生的事——你开始把 Transformer 看作非常巧妙的数学，而不是魔法。

在评论里留下你的想法、困惑，甚至“原来如此！”的瞬间——我很想知道这一切在你脑子里是怎么（或者没能）豁然开朗的👇

## 延伸阅读

感谢阅读！如果你想看我更多的文字，这里有一篇文章把我其余的博客按主题整理在了一起：数据科学团队与项目管理、数据叙事、营销与竞价科学，以及机器学习与建模。

## 敬请关注！

如果你想在我发布新内容时收到通知，欢迎在 Medium 上关注我。另外，[我也很乐意在 LinkedIn 上和你聊聊](https://medium.com/data-science-collective/www.linkedin.com/in/joseparrenogarcia)！
